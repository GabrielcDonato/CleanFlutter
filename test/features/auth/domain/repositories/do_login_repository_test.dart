...
import 'package:clean_architecture_experts_club/features/auth/data/repositories/do_login_repository_impl.dart';
...
void main() {
  ...
  test('Deve realizar o login', () async {
    when(() => _dataSource.doLogin(email: any(named: "email"), password: any(named: "password"))).thenAnswer((invocation) async => tUser);

    var result = await _repository.doLogin(email: 'email@email.com', password: 'password123');

    expect(result, isA<Right>());
    expect(result, Right(tUser));

    verify(() => _dataSource.doLogin(email: any(named: "email"), password: any(named: "password"))).called(1);

    verifyNoMoreInteractions(_dataSource);
  });

  test('Deve obter errro ao realizar o login', () async {
    when(() => _dataSource.doLogin(email: any(named: "email"), password: any(named: "password"))).thenThrow(PasswordWrongFailure());
...
var tUser = UserModel(bornDate: DateTime.now(), email: 'email@email.com', name: 'Nome Usuário', pictureUrl: 'https://pictureuser.com.br/photo');
